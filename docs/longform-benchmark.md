...
Novel Generator includes a deterministic stress benchmark for the long-form context architecture. It does not call an LLM. Instead, it constructs a deliberately difficult synthetic 32-chapter manuscript and verifies that the deterministic systems still retrieve, bound, prioritize, and converge the correct state as the book grows.
...
- enough early under-writing to put the manuscript behind its whole-book word target;
- unresolved promises and emotional debt close to the ending; and
- enough chapters/prose to force bounded developmental and manuscript-QA representations.

The benchmark currently verifies eight cross-cutting properties:
...
7. the developmental manuscript map keeps every completed chapter while staying bounded; and
8. the manuscript-QA map also keeps every completed chapter while staying bounded.

This is intentionally an architectural benchmark, not a claim that deterministic checks can judge prose quality. Its purpose is to catch regressions where a future code change quietly drops an old callback, stops detecting abandoned arcs, lets late-book story debt multiply again, or loses whole-book coverage under context pressure.
